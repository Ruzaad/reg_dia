-- PARCHE 62 — Cache de tickets_area (fix de los timeouts de 3-8s en fn_tickets_area)
-- fn_tickets_of_raw recalculaba TODO el catálogo de tickets del área en cada
-- llamada (37,531 filas para CAMISA COSTURA, ~2.8s solo de cómputo). Ahora se
-- guarda en tickets_cache y solo se recalcula cuando ingeniería cambia algo
-- que afecta el catálogo (bases, troceo, OF generada/registrada).

create table if not exists tickets_cache (
  area text not null,
  codigo text not null,
  o_f text, articulo text, modulo text, op text, n_op integer, op_id bigint,
  std numeric, cant numeric, paq integer, talla text, color text,
  desde integer, hasta integer,
  actualizado timestamptz not null default now(),
  primary key (area, codigo)
);
create index if not exists tickets_cache_area_idx on tickets_cache(area);
alter table tickets_cache enable row level security;

create or replace function fn_tickets_cache_refrescar(p_area text) returns void
...
begin
  ...
  select p_area, codigo, o_f, articulo, modulo, op, n_op, op_id,
    std, cant, paq, talla, color, desde, hasta
  from fn_tickets_of_raw(p_area, null);
end $$;

-- fn_tickets_area: mismo contrato (mismo shape de salida), ahora lee de la
-- cache en vez de recalcular.
create or replace function fn_tickets_area(p_dni text, p_token uuid, p_area text)
 returns json
 language plpgsql
 security definer
...
-- ---- Invalidación: un `perform fn_tickets_cache_refrescar(...)` agregado a
-- ---- cada función que escribe en bases / of_troceo / of_detalle / of_generada.
...
begin
  ...
  perform fn_tickets_cache_refrescar(p_area);
...
begin
  ...
  perform fn_tickets_cache_refrescar(v_area);
...
declare o operarios; arts text[]; v_prop int := 0;
        v_new int := 0; v_upd int := 0; v_del int := 0; v_conflicto text;
begin
...
-- fn_of_registrar: red de seguridad. Solo re-registra desglose/cantidad de
-- una OF (previo a fn_of_generar en el flujo normal), pero si la OF YA fue
-- generada en alguna área (re-subida de HN), refresca esas áreas también.
  ...
  if found then
    ...
    if not v_hay and v_pre <> '' then
      ...
      for v_area_gen in select area from of_generada where o_f = v_of loop
        perform fn_tickets_cache_refrescar(v_area_gen);
      end loop;
...
-- Backfill: la cache nace vacía, se llena una vez para las áreas ya generadas.
select fn_tickets_cache_refrescar(area) from (select distinct area from of_generada) x;
